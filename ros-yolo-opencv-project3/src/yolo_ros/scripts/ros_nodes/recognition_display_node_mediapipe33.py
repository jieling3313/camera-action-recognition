...
# -*- coding: utf-8 -*-
"""
即時辨識顯示節點 v3 - MediaPipe 33 點版本

使用 MediaPipe 33 點骨架格式進行動作辨識，支援：
1. One-Shot Learning: 使用 Page 2 設定的自訂動作
2. NTU RGB+D Classification: 使用預訓練模型的 60 個類別

與 v2 版本的主要差異：
- 使用 MediaPipe 33 點而非 COCO 17 點
- 使用 MediaPipeSkeletonEmbedding 模型
- 不需要 MediaPipe -> COCO 轉換
...
sys.path.insert(0, os.path.join(models_dir, 'mediapipe33'))
sys.path.insert(0, os.path.join(models_dir, 'common'))

try:
    from skeleton_model_mediapipe33 import MediaPipeSkeletonEmbedding
    MODEL_AVAILABLE = True
except ImportError as e:
    rospy.logerr(f"Failed to import MediaPipe33 model: {e}")
...
class RecognitionDisplayNodeMediaPipe33:
    """即時辨識顯示節點 - MediaPipe 33 點版本"""

    def __init__(self):
        """初始化辨識顯示節點"""
        rospy.init_node('recognition_display_node_mediapipe33', anonymous=False)
        ...
        # NTU RGB+D Classification
        self.use_ntu_classification = True  # 啟用 NTU 分類
        ...
        rospy.loginfo("=" * 60)
        rospy.loginfo("Recognition Display Node (MediaPipe 33-Point) initialized")
        rospy.loginfo("=" * 60)
        rospy.loginfo(f"Device: {self.device}")
        rospy.loginfo(f"Skeleton format: MediaPipe 33 points")
        rospy.loginfo(f"Buffer size: 64 frames")
    ...
    def load_model(self):
        """載入預訓練的 MediaPipe 33 點骨架辨識模型"""
        if not MODEL_AVAILABLE:
            rospy.logerr("MediaPipe33 model not available")
            return

        # 嘗試多個可能的 checkpoint 路徑
        checkpoint_paths = [
            "/root/catkin_ws/src/yolo_ros/scripts/checkpoints_mediapipe33/best.pth",
            "/root/catkin_ws/src/yolo_ros/scripts/models/mediapipe33/checkpoints/best.pth",
            "/root/catkin_ws/src/yolo_ros/scripts/checkpoints/best_mediapipe33.pth",
        ]
        ...
        if checkpoint_path is None:
            rospy.logwarn("No MediaPipe33 checkpoint found at:")
            for path in checkpoint_paths:
                rospy.logwarn(f"  - {path}")
            rospy.logwarn("Please train the model first using train_mediapipe33.py")
        ...
        try:
            ...
            if 'model_state_dict' in checkpoint:
                ...
                if 'classifier.weight' in state_dict:
                    ...
                    rospy.loginfo(f"Model trained with {num_classes} classes")

            # 建立 MediaPipe 33 點模型實例
            self.model = MediaPipeSkeletonEmbedding(
                in_channels=3,
                base_channels=64,
            ...
            rospy.loginfo(f"Model loaded successfully from: {checkpoint_path}")
            rospy.loginfo(f"  Epoch: {checkpoint.get('epoch', 'N/A')}")
            rospy.loginfo(f"  Best Acc: {checkpoint.get('best_acc', 'N/A')}%")
        ...
        try:
            ...
            if rospy.has_param('/model_manager/custom_actions'):
                ...
                if custom_actions and support_features_dict:
                    ...
                    rospy.loginfo("No custom actions found in ROS parameters")
            else:
                rospy.loginfo("No custom actions parameter found")
    ...
    def handle_load_actions(self, req):
        ...
        try:
            ...
            if rospy.has_param('/model_manager/custom_actions'):
                ...
                rospy.loginfo(f"Loaded custom actions: {self.custom_actions}")
            else:
                self.custom_actions = []
            ...
            rospy.logerr(f"Failed to load actions: {e}")
    ...
    def recognize_ntu_classification(self, skeleton_sequence):
        ...
        try:
            ...
            # 轉換為 tensor
            tensor = torch.FloatTensor(skeleton_norm).unsqueeze(0).to(self.device)  # (1, 64, 33, 3)
            ...
            with torch.no_grad():
                ...
                for i, prob in enumerate(probs.cpu().numpy()):
                    if i < len(NTU_60_ACTIONS):
                        class_name = NTU_60_ACTIONS[i]
    ...
    def extract_mediapipe_keypoints(self, landmarks, image_shape):
        """從 MediaPipe landmarks 提取 33 點骨架

        Args:
            landmarks: MediaPipe pose landmarks
            image_shape: (height, width) 影像尺寸

        Returns:
            numpy array (33, 3) - x, y, visibility
        """
        h, w = image_shape[:2]
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            if results.pose_landmarks:
                ...
                # 提取 MediaPipe 33 關鍵點（直接使用，不轉換為 COCO）
                keypoints = self.extract_mediapipe_keypoints(
                ...
                if len(self.skeleton_buffer) >= 64 and self.frame_count % self.recognition_interval == 0:
                    skeleton_sequence = np.array(list(self.skeleton_buffer))  # (64, 33, 3)

                    # 執行雙模式辨識
                    all_scores = {}

                    # 1. One-Shot Learning（自訂動作）
                    if self.custom_actions and self.support_features and self.model is not None:
                        # 提取特徵向量
                    ...
                    if self.use_ntu_classification and self.model is not None:
    ...
    def _draw_recognition_results(self, image):
        ...
        if self.all_scores:
            ...
            cv2.putText(image, "Top 5:", (text_x, text_y + 90), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1)

            for i, (action, score) in enumerate(sorted_scores):
                y_pos = text_y + 115 + i * 22
                color = (0, 255, 0) if i == 0 else (180, 180, 180)
                text = f"{i+1}. {action[:28]}: {score:.1f}%"
                cv2.putText(image, text, (text_x + 10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.45, color, 1)

        # 顯示緩衝區狀態
        buffer_status = f"Buffer: {len(self.skeleton_buffer)}/64"
        cv2.putText(image, buffer_status, (x_start + 10, y_end - 10),
...
if __name__ == '__main__':
    try:
        node = RecognitionDisplayNodeMediaPipe33()
        node.run()
