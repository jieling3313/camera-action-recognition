...
class Pose:
    ...
    def __init__(
        ...
        # 創建 PoseLandmarker
        # 模型路徑：優先使用 workspace 中的持久化模型（不會因 docker-compose down 消失）
        import os
        model_paths = [
            '/root/catkin_ws/src/yolo_ros/models/mediapipe/pose_landmarker.task',  # workspace mount
            '/root/.mediapipe/models/pose_landmarker.task',  # fallback
        ]
        model_path = None
        ...
        if model_path is None:
            raise FileNotFoundError(
                ...
                f"Please download it to: {model_paths[0]}"
            )
...
class DrawingSpec:
    """繪製規格類（模擬 mp.solutions.drawing_utils.DrawingSpec）"""
...
class DrawingUtils:
    ...
    def draw_landmarks(
        ...
        if landmark_drawing_spec is None:
            landmark_drawing_spec = DrawingSpec(color=(0, 0, 255), thickness=2, circle_radius=5)
        if connection_drawing_spec is None:
            connection_drawing_spec = DrawingSpec(color=(0, 255, 0), thickness=2)
...
class DrawingStyles:
    """兼容舊版 mp.solutions.drawing_styles"""

    @staticmethod
    def get_default_pose_landmarks_style():
        """返回預設的姿勢關鍵點繪製樣式"""
        return DrawingSpec(color=(0, 0, 255), thickness=2, circle_radius=3)
